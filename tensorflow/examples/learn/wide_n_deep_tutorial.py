...
def input_fn(file_names, batch_size):
  ...
  df[LABEL_COLUMN] = df["income_bracket"].apply(lambda x: ">50K" in x)
...
def train_and_eval(model_dir, model_type, train_steps, train_data, test_data, n_classes, batch_size):
  ...
  m.fit(input_fn=lambda: input_fn([train_file_name], batch_size), steps=train_steps, monitors=[validation_monitor])
  results = m.evaluate(input_fn=lambda: input_fn([test_file_name], batch_size), steps=1)
